...
> categories and **23** edge predicates and makes **provenance node-based**: an edge's
...
2. **Every relationship is a typed `edges:` entry whose `predicate` is one of these 23:**
   ...
   `affects_response_to`, `has_phenotype`, `measures`, `associated_with`, `reported_in`.
   Direction is always **this document → object**. The 23 are **forward-only** — there are no
...
## Edges: domain/range notes (predicates unchanged at 23)

The 23 predicates are unchanged across all versions. The 28 node types ride existing
predicates via domain/range extensions — the key ones:
