...
missing_scripts = [src for src in script_paths if not (studio / "dist" / src).is_file()]
if missing_scripts:
    print("FAIL: index.html references missing local scripts:")
    for src in missing_scripts:
        print("  " + src)
    sys.exit(1)
js = "\n".join((studio / "dist" / src).read_text() for src in script_paths)
main_rs = (studio / "src-tauri" / "src" / "main.rs").read_text()

failed = False

# Every interactive element with an id must be referenced by one of the scripts
# index.html actually loads. A control nothing reads is a control that does
# nothing when clicked.
ids = re.findall(r"<(?:button|a|input|select|textarea)\b[^>]*\bid=\"([^\"]+)\"", html)
dynamic_prefixes = set()
for values in re.findall(
    r"\[((?:\s*['\"][a-z0-9-]+['\"]\s*,?)+)\]\.forEach\((?:modelOptions|bindProvider)\)",
    js,
):
    dynamic_prefixes.update(re.findall(r"['\"]([a-z0-9-]+)['\"]", values))
dynamic_suffixes = set(re.findall(r"prefix\s*\+\s*['\"](-[a-z0-9-]+)['\"]", js))
dynamic_ids = {prefix + suffix for prefix in dynamic_prefixes for suffix in dynamic_suffixes}
dead = [i for i in ids if i not in js and i not in dynamic_ids]
if dead:
    failed = True
    print("FAIL: declared in index.html but never referenced by a loaded script:")
    for i in dead:
        print("  " + i)

# Every Tauri command a loaded script invokes must be registered in main.rs, or
# the call fails at runtime with no compile-time signal.
...
if unregistered:
    failed = True
    print("FAIL: a loaded script invokes Tauri commands not registered in main.rs:")
    for c in unregistered:
        print("  " + c)

if failed:
    sys.exit(1)
print(f"wiring OK ({len(ids)} controls, {len(called)} commands, {len(script_paths)} scripts)")
PY
