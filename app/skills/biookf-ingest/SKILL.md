...
You are curating a **BioOKF v0.5** bundle. Drive the `biookf` MCP tools (or the `bokf` CLI). You own `knowledge/`, `index.md`, and `log.md`; **never edit `raw/`**. (Step 0, **biookf-convert**, has already put each source under `raw/` as faithful Markdown.)

## Ground rules (apply throughout)
- **Source-only.** Every claim, edge, and `xref` comes from **the source document plus your own knowledge** of what the named entity is. Do **not** call external tools (web, PubMed, other databases) to mine new claims or resolve identifiers while ingesting; an `xref` you can't fill from the source is left for a later enrichment pass (a missing `xref` is **never** an error).
- **Classify by identity, not role.** Type each entity by what it *is* (aspirin *is* a `Molecule`; "treats headache" is an edge). For ambiguous types, use the tiebreakers in [`docs/04-classification_guidelines.md`](../../../docs/04-classification_guidelines.md): disambiguate-by-use (a word whose referent changes → classify by its use here), identity-over-role, subtype-for-specificity (coarsest still-useful granularity); the **Disease / Phenotype / BiomedicalMeasure** trio are distinct facets (one node each, linked by edges). When genuinely nothing fits, `Other` + a `note:`; never invent a type.
- **Mint the right node; reuse before forking.** A missing node is a reason to create it, never to bend a concept onto the nearest existing one. But before creating, check `index.md`'s identifier registry (and open the candidate `.md` to confirm); reuse an existing `subtype` from the subtypes-in-use list when one fits.
...
3. **Extract typed entities.** For each biomedical thing discussed in the text or shown in a figure, create or UPDATE a concept doc. `type` MUST be one of the 28; coin a lowercase `subtype`. **Reuse an existing `identifier`, never fork** (search first with `bokf_search`). When the entity already exists, **UPDATE** it: add a `reported_in` edge to this source and merge in genuinely new synonyms / `xref` / description **without discarding** what is already there. Likewise don't duplicate an edge: if the same `predicate` + `object` already exists from another source, add this source (extend `publications` / add a parallel provenance-stamped edge) and merge in distinct statistics.
