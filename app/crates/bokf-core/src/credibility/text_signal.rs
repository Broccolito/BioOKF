...
pub fn scholarly_text_signal(
    text: &str,
    ids: &SourceIds,
) -> Option<(SourceType, CredibilityTier, f32)> {
    ...
    let marker_count = JOURNAL_MARKERS
        .iter()
...
mod tests {
    ...
    fn text_signal_detects_journal() {
        let ids = SourceIds {
            doi: Some("10.x/y".into()),
            ..Default::default()
        };
        let t = "Received: 1 Jan. Accepted: 2 Feb. Corresponding author: x. Journal of Things.";
        assert!(matches!(
            scholarly_text_signal(t, &ids),
            Some((_, CredibilityTier::PeerReviewed, _))
        ));
    }
}
