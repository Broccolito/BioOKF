...
fn lint_inner(bundle: &Bundle, include_pdf_coverage: bool) -> LintReport {
    ...
    for n in &bundle.nodes {
        ...
        if !n.node_type.is_valid() {
            r.push(
                ...
                &n.identifier,
                format!(
                    "`{}` is not one of the 28 controlled node types",
                    n.raw_type
                ),
                path.clone(),
            );
        ...
        if n.node_type.is_provenance() && n.raw_source.is_empty() && n.xref.is_empty() {
            r.push(
                ...
                "source.unanchored",
                &n.identifier,
                "provenance node has neither a `raw_source` path nor an external `xref`"
                    .to_string(),
                path.clone(),
            );
        }
            ...
            .and_then(|s| s.to_str())
        {
            ...
            if is_type_dir
                && n.node_type.is_valid()
                && !n.node_type.as_str().eq_ignore_ascii_case(&dir_l)
            {
                r.push(
                    Severity::Warn,
                    "type.path_mismatch",
        ...
        for e in &n.edges {
            let key = (
                ...
                e.object.clone(),
                e.primary_source.clone().unwrap_or_default(),
            );
        ...
        if matches!(n.node_type, NodeType::Other) && n.note.is_none() {
            r.push(
                ...
                "other.missing_note",
                &n.identifier,
                "type `Other` requires a `note:` explaining why no controlled type fits"
                    .to_string(),
                path.clone(),
            );
        }
        ...
        if !n.node_type.is_provenance()
            && !n
                .edges
                .iter()
                .any(|e| matches!(e.predicate.base(), Predicate::ReportedIn))
        {
            r.push(
                Severity::Warn,
                "node.no_reported_in",
                &n.identifier,
                "node has no `reported_in` edge linking it to a source".to_string(),
                path.clone(),
            );
        }
        ...
        for rs in &n.raw_source {
            match crate::export::confine_to_bundle(&bundle.root, rs) {
                ...
                Ok(_) => {
                    r.push(
                        Severity::Warn,
                        "source.raw_missing_file",
                        &n.identifier,
                        format!("raw_source `{rs}` does not exist under the bundle"),
                        path.clone(),
                    );
                }
                Err(_) => {
                    r.push(
                        Severity::Warn,
                        "source.raw_escapes_bundle",
                        &n.identifier,
                        format!("raw_source `{rs}` escapes the bundle (absolute or `..`)"),
                        path.clone(),
                    );
                }
            }
    ...
    for id in graph.orphans() {
        r.push(
            ...
            "node.orphan",
            &id,
            "node has no edges (orphan)".to_string(),
            None,
        );
    }
    ...
    if bundle.has_index_md {
        for id in crate::index::missing_from_index(bundle) {
            r.push(
                ...
                "index.stale",
                &id,
                "identifier is not registered in index.md; run `bokf index`".to_string(),
                None,
            );
        }
    }
...
fn lint_edge(r: &mut LintReport, bundle: &Bundle, n: &Node, e: &Edge, path: &Option<String>) {
    ...
    if !e.predicate.is_valid() {
        r.push(
            ...
            subj,
            format!(
                "`{}` is not one of the {} controlled predicates",
                e.raw_predicate,
                PREDICATES.len()
            ),
            path.clone(),
    ...
    if e.reversed {
        r.push(
            ...
            subj,
            format!(
                "`{}` is a deprecated inverse alias; author the forward `{}` on the other node",
                e.raw_predicate,
                e.predicate.as_str()
            ),
            path.clone(),
    ...
    match &e.knowledge_level {
        None => r.push(
            Severity::Error,
            "edge.missing_knowledge_level",
            subj,
            format!(
                "edge `{} -> {}` missing knowledge_level",
                e.predicate.as_str(),
                e.object
            ),
            path.clone(),
        ),
        Some(v) if !KNOWLEDGE_LEVELS.contains(&v.as_str()) => r.push(
            Severity::Error,
            "edge.invalid_knowledge_level",
            subj,
            format!("invalid knowledge_level `{v}`"),
            path.clone(),
        ),
        _ => {}
    }
    match &e.agent_type {
        None => r.push(
            Severity::Error,
            "edge.missing_agent_type",
            subj,
            format!(
                "edge `{} -> {}` missing agent_type",
                e.predicate.as_str(),
                e.object
            ),
            path.clone(),
        ),
        Some(v) if !AGENT_TYPES.contains(&v.as_str()) => r.push(
            Severity::Error,
            "edge.invalid_agent_type",
            subj,
            format!("invalid agent_type `{v}`"),
            path.clone(),
        ),
        _ => {}
    }
    ...
    if !bundle.contains(&e.object) {
        ...
        r.push(
            ...
            subj,
            format!(
                "edge object `{}` does not resolve to any node's identifier",
                e.object
            ),
            path.clone(),
        );
    ...
    if matches!(e.predicate, Predicate::Regulates | Predicate::ExpressedIn) && e.direction.is_none()
    {
        r.push(
            Severity::Info,
            "edge.missing_direction",
            subj,
            format!(
                "`{}` should carry a `direction` (increased/decreased)",
                e.predicate.as_str()
            ),
            path.clone(),
        );
    ...
    if let (Some(lo), Some(hi)) = (stat_num(e, "ci_lower"), stat_num(e, "ci_upper")) {
        if lo > hi {
            r.push(
                ...
                "edge.stat_ci",
                subj,
                format!("ci_lower ({lo}) > ci_upper ({hi})"),
                path.clone(),
            );
        }
    }
    if let Some(p) = stat_num(e, "p_value") {
        if !(0.0..=1.0).contains(&p) {
            r.push(
                ...
                "edge.stat_pvalue",
                subj,
                format!("p_value {p} is outside [0, 1]"),
                path.clone(),
            );
        }
    }
...
fn looks_like_measurement_value(id: &str) -> bool {
    ...
    rest.is_empty()
        || rest == "%"
        || (rest.len() <= 3 && rest.chars().all(|c| c.is_ascii_alphabetic()))
}
        ...
        Predicate::Treats | Predicate::Prevents => {
            if !matches!(obj.node_type, Disease | Phenotype) {
                warn(
                    r,
                    format!(
                        "`{}` should target a Disease/Phenotype, but `{}` is a {}",
                        e.predicate.as_str(),
                        e.object,
                        obj.node_type.as_str()
                    ),
                );
            }
        }
        Predicate::HasPhenotype => {
            if !matches!(obj.node_type, Phenotype) {
                warn(
                    r,
                    format!(
                        "`{}` should target a Phenotype, but `{}` is a {}",
                        e.predicate.as_str(),
                        e.object,
                        obj.node_type.as_str()
                    ),
        ...
        Predicate::Encodes => {
            if !matches!(obj.node_type, Molecule) {
                warn(
                    r,
                    format!(
                        "`encodes` should target a Molecule, but `{}` is a {}",
                        e.object,
                        obj.node_type.as_str()
                    ),
                );
    ...
    for n in &bundle.nodes {
        for e in &n.edges {
            ...
            if matches!(
                ...
                CredibilityTier::Web | CredibilityTier::Unknown
            ) {
                r.push(
                    Severity::Warn,
            ...
            if meta.credibility.retracted {
                r.push(
                    ...
                    &src.identifier,
                    format!(
                        "source `{}` is cited as primary_source but is marked retracted",
                        src.identifier
                    ),
                    None,
                );
...
fn lint_contradictions(r: &mut LintReport, bundle: &Bundle) {
    ...
    for n in &bundle.nodes {
        for e in &n.edges {
            let key = (
                n.identifier.clone(),
                e.predicate.base().as_str().to_string(),
                e.object.clone(),
            );
    ...
    for ((subj, pred, obj), (pos, neg)) in seen {
        if pos && neg {
            r.push(
                ...
                &subj,
                format!(
                    "contradictory edges: both `{pred}` and `not_{pred}` are asserted for `{obj}`"
                ),
                None,
            );
        }
...
mod tests {
    ...
    fn lints_flag_not_scholarly_and_retracted_sources() {
        ...
        std::fs::write(
            root.join("raw/web-src/meta.yaml"),
        ...
        assert!(
            rep.findings
                .iter()
                .any(|f| f.rule == "source.not_scholarly"),
            "{:?}",
            rep.findings
        );
        ...
        assert!(
            rep2.findings.iter().any(|f| f.rule == "source.retracted"),
            "{:?}",
            rep2.findings
        );
        assert!(!rep2
    ...
    fn lints_flag_unnamed_and_undescribed_figures() {
        ...
        assert!(rep
            .findings
            .iter()
            .any(|f| f.rule == "source.figure_unnamed"));
        assert!(rep
            .findings
            .iter()
            .any(|f| f.rule == "source.figure_undescribed"));
    }
    ...
    fn used_to_study_range_violation_warns() {
        ...
        assert!(
            r.findings
                .iter()
                .any(|f| f.rule == "edge.range" && f.message.contains("used_to_study")),
    ...
    fn flags_duplicate_edge_same_source() {
        ...
        let r = lint_docs(&[
            ("gene/braf.md", gene),
            ("disease/melanoma.md", disease),
            ("publication/demo.md", src),
        ]);
        assert!(
            r.findings.iter().any(|f| f.rule == "edge.duplicate"),
            "{:?}",
            r.findings
        );
    }
    ...
    fn flags_type_path_mismatch() {
        ...
        assert!(
            r.findings.iter().any(|f| f.rule == "type.path_mismatch"),
            "{:?}",
            r.findings
        );
    }
    ...
    fn flags_similar_subtypes() {
        ...
        assert!(
            r.findings
                .iter()
                .any(|f| f.rule == "subtype.similar" && f.message.contains("protein")),
            "{:?}",
            r.findings
        );
    }
    ...
    fn not_predicate_is_valid_and_range_checked() {
        ...
        let src =
            "---\ntype: Study\nidentifier: Trial\nsubtype: rct\nraw_source: [raw/t]\n---\n# t\n";
        let r = lint_docs(&[
            ("molecule/drugx.md", drug),
            ("gene/braf.md", gene),
            ("study/trial.md", src),
        ]);
        assert!(
            !r.findings.iter().any(|f| f.rule == "predicate.invalid"),
            "{:?}",
            r.findings
        );
        assert!(
            r.findings
                .iter()
                .any(|f| f.rule == "edge.range" && f.message.contains("not_treats")),
            "{:?}",
            r.findings
        );
    }
    ...
    fn negated_on_nonnegatable_predicate_errors() {
        ...
        let r = lint_docs(&[
            ("gene/braf.md", g),
            ("molecularclass/oncogene.md", oncogene),
            ("publication/src.md", src),
        ]);
        assert!(
            r.findings.iter().any(|f| f.rule == "edge.not_negatable"),
            "{:?}",
            r.findings
        );
    }
    ...
    fn positive_and_negative_edge_contradict() {
        ...
        let r = lint_docs(&[
            ("molecule/drugx.md", drug),
            ("disease/asthma.md", dis),
            ("study/a.md", a),
            ("study/b.md", b),
        ]);
        assert!(
            r.findings.iter().any(|f| f.rule == "edge.contradiction"),
            "{:?}",
            r.findings
        );
    }
    ...
    fn value_as_identifier_flagged_but_not_real_entities() {
        ...
        assert!(
            r.findings
                .iter()
                .any(|f| f.rule == "value.as_identifier" && f.subject == "183 cm"),
            "{:?}",
            r.findings
        );
        assert!(
            !r.findings
                .iter()
                .any(|f| f.rule == "value.as_identifier" && f.subject == "2-AG"),
            "{:?}",
            r.findings
        );
    }
    ...
    fn other_requires_note() {
        ...
        assert!(
            r.findings.iter().any(|f| f.rule == "other.missing_note"),
            "{:?}",
            r.findings
        );
    }
    ...
    fn ci_ordering_flagged() {
        ...
        assert!(
            r.findings.iter().any(|f| f.rule == "edge.stat_ci"),
            "{:?}",
            r.findings
        );
    }
