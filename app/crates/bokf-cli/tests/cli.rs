...
fn scaffold_lint_graph_search_roundtrip() {
    ...
    let out = Command::new(bokf())
        .args(["scaffold", dir.to_str().unwrap(), "--name", "Test KB"])
    ...
    write(
        &dir.join("knowledge/publication/src.md"),
        r#"---
type: Publication
...
"#,
    ...
    write(
        &dir.join("knowledge/gene/braf.md"),
        r#"---
type: Gene
identifier: BRAF
...
"#,
    ...
    write(
        &dir.join("knowledge/disease/melanoma.md"),
        r#"---
type: Disease
identifier: Melanoma
subtype: neoplasm
...
"#,
    ...
    assert!(edges.iter().any(|e| e["source"] == "BRAF"
        && e["target"] == "Melanoma"
        && e["predicate"] == "associated_with"));

    // search finds BRAF
    let out = Command::new(bokf())
        .args(["search", dir.to_str().unwrap(), "kinase", "--json"])
        .output()
        .unwrap();
    ...
    assert!(hits
        ...
        .unwrap()
        .iter()
        .any(|h| h["identifier"] == "BRAF"));

    // topology metrics default to the biological projection, excluding provenance nodes
    let out = Command::new(bokf())
        .args(["network-metrics", dir.to_str().unwrap()])
        .output()
        .unwrap();
    assert!(
        out.status.success(),
        "network-metrics failed: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    let metrics: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    assert_eq!(metrics["global"]["nodes"], 2);
    assert_eq!(metrics["global"]["edges"], 1);
    assert_eq!(metrics["excluded_provenance"], true);

    // now introduce an INVALID type + a bad predicate -> lint must flag errors and exit 1
    write(
        &dir.join("knowledge/other/bad.md"),
        r#"---
type: NotAType
identifier: Bad node
edges:
...
"#,
    ...
    let rules: Vec<String> = report["findings"]
        ...
        .unwrap()
        .iter()
        .map(|f| f["rule"].as_str().unwrap_or("").to_string())
        .collect();
    assert!(
        rules.iter().any(|r| r == "type.invalid"),
        "should flag invalid type"
    );
    assert!(
        rules.iter().any(|r| r == "predicate.invalid"),
        "should flag invalid predicate"
    );
    assert!(
        !out.status.success(),
        "bundle with errors should exit nonzero"
    );
...
#[test]
fn agent_workflows_are_exposed_with_unambiguous_limits() {
    let root = Command::new(bokf()).arg("--help").output().unwrap();
    ...
    let generated = Command::new(bokf())
        .args(["generate-from-paperclip", "--help"])
        .output()
        .unwrap();
    let help = String::from_utf8_lossy(&generated.stdout);
    assert!(help.contains("--max-per-source"));
    assert!(help.contains("Maximum number of results from each selected database"));
    assert!(!help.contains("papers/database"));
}
...
fn cli_log_sync_then_log() {
    ...
    let log = Command::new(bokf())
        .args(["log", dir.path().to_str().unwrap(), "--json"])
        .output()
        .unwrap();
...
fn scaffold_registers_into_config_dir_not_parent() {
    ...
    assert!(
        cfg.path().join("registry.yaml").exists(),
        "registry.yaml should be in config dir"
    );
    // ...and NOT scattered next to the new KB.
    assert!(
        !workdir.path().join("registry.yaml").exists(),
        "must not scatter registry to parent"
    );
    assert!(
        !workdir.path().join(".active-kb").exists(),
        "must not scatter .active-kb to parent"
    );
...
fn verify_gate_passes_clean_fails_dirty() {
    ...
    let out = Command::new(bokf())
        .args(["verify", dir.to_str().unwrap(), "--json"])
        .output()
        .unwrap();
    let v: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
...
fn end_to_end_scaffold_write_logsync_log_restore() {
    ...
    assert!(
        run(&["scaffold", bundle.to_str().unwrap(), "--name", "Demo"])
            .status
            .success()
    );
    ...
    let first_sha = entries.as_array().unwrap().last().unwrap()["commit_sha"]
        .as_str()
        .unwrap()
        .to_string();

    // restore to the scaffold commit: IL6 is gone, history grew with a restore entry
    assert!(run(&["restore", bundle.to_str().unwrap(), &first_sha])
        .status
        .success());
