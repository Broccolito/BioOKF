...
Each claim's `primary_source` names a source node by identifier. A source node is either an ingested document (a Publication/Study/Dataset with `raw_source: [raw/...]`) or an external reference (e.g. HGNC, Gene Ontology, DrugBank: a node with its `infores:`/ontology CURIE in `xref`, no raw_source). Create each external-reference source node ONCE and reuse its identifier. Also add a `reported_in` edge to make provenance traversable. (By convention a `reported_in` edge's own `primary_source` is its own `object`.)

CLASSIFY BY IDENTITY, NOT ROLE. Type each entity by what it IS, not what it does here (aspirin IS a Molecule; "treats headache" is an edge). Mint the right node; never bend a concept onto the nearest existing node because the correct one is missing. A missing node is a reason to create it. For ambiguous types use the classification tiebreakers in `docs/04-classification_guidelines.md`: disambiguate-by-use (a word whose referent changes -> classify by its use here), identity-over-role, subtype-for-specificity (coarsest still-useful granularity); the Disease / Phenotype / BiomedicalMeasure trio are distinct facets (one node each, linked by edges). Reuse an existing `subtype` from index.md before coining a new one.

SOURCE-ONLY (during ingestion). Every claim, edge, and `xref` comes from the source document plus your own knowledge of what the named entity is. Do NOT call external tools (web search, PubMed, other databases) to mine new claims or resolve identifiers while ingesting; an `xref` you cannot fill from the source is left for a later enrichment pass (a missing `xref` is never an error). Reuse an existing node before creating one: check index.md's identifier registry and open the candidate's `.md` to confirm; when the entity already exists, UPDATE it: add a `reported_in` edge to this source and merge in genuinely new synonyms / `xref` / description WITHOUT discarding what is there, and never fork a duplicate. The same "combine, never discard; keep both on direct contradiction" rule governs merging two bundles.
...
7. Update index.md (bokf_write_page) and commit the step with bokf_log_sync, using --delta for any count summary.
