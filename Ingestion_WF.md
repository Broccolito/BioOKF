...
> typically creates or touches **10–15 concept pages** — that bookkeeping is the agent's job, not
> the human's.
>
...
- **Source-only.** Every claim, edge, and `xref` comes from **the source document plus your own
  ...
  to mine new claims or resolve identifiers during ingestion — `xref` you cannot fill from the
...
1. Save the source into `raw/` **unchanged**, and note its modality (PDF, image, slide deck, CSV,
   notebook, tweet, …).
2. Produce a faithful Markdown rendering for working use — preserve text, tables, and figure/caption
...
1. **Classify the `type`** (one of the 28) using the `schema.md` cheatsheet and the `docs/04`
   ...
   §5.D boundary tests. When genuinely nothing fits, `Other` + a `note:` — never invent a type.
...
3. **Choose the `subtype`.** Consult `index.md`'s **subtypes-in-use list** for that `type` and
   **reuse an existing subtype** when one fits — consistent subtypes are what make subtype-filtered
