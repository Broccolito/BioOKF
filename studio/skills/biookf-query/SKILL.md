...
## The loop
1. Read `index.md` for orientation (`okf_read_page`).
2. **Search broadly, then traverse.** `okf_search` (CLI: `okf search`) ranks by BM25 — it finds
...
3. `okf_read_page` the top hits; read their `edges:`.
4. Traverse with `okf_graph` (CLI: `okf graph`) to find neighbors (graph-shaped reasoning, e.g.
...
- Surface contradictions if `okf_lint` reports them for the entities involved.
