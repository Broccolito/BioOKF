...
**Goal:** Reconcile the predicate set to the authoritative 24, add git-backed version tracking (with a single step-committer that keeps `log.md` and git in lockstep), and add active-KB context — all as `okf-core`/`okf-cli`/`okf-mcp` primitives.

**Architecture:** Pure logic stays in `okf-core` (new modules `git.rs`, `log_sync.rs`, `registry.rs`, `active.rs`, plus a `model.rs` edit). The CLI (`okf`) and MCP server (`okf-mcp`, server name `biookf`) are thin front-ends. Git is driven by **shelling out to the system `git` binary** (`std::process::Command`) — no `git2`/libgit2 dependency.

**Tech Stack:** Rust 2021 workspace at `studio/`; crates `okf-core`, `okf-cli` (clap), `okf-mcp` (rmcp); `serde`/`serde_yaml`/`serde_json`; new direct dep `uuid` (v4, already in lockfile), new dev-dep `tempfile`.

## Global Constraints

- Predicate set is **24** (`schema.md` authoritative): the 23 core **+ `used_to_study`**. One source of truth: `okf_core::model::PREDICATES`.
...
- **`log_sync` is the sole step-committer** (log.md append + commit, atomic). The lower-level `okf commit` is for non-logged lifecycle commits only. There is **no** per-tool auto-commit.
...
|---|---|---|
| `crates/okf-core/src/model.rs` | controlled vocab incl. the 24th predicate | modify |
| `crates/okf-core/src/lib.rs` | module decls, re-exports, vocab assert (24) | modify |
| `crates/okf-core/src/lint.rs` | `used_to_study` domain/range + "24" message | modify |
| `crates/okf-core/src/git.rs` | `GitRepo`, `ChangeKind`, `HistoryEntry`, date helper | create |
| `crates/okf-core/src/log_sync.rs` | atomic log.md append + commit (sole step-committer) | create |
| `crates/okf-core/src/registry.rs` | known-bundle registry + `validate_kb_id` | create |
| `crates/okf-core/src/active.rs` | active-KB pointer | create |
| `crates/okf-core/Cargo.toml` | add `uuid` dep + `tempfile` dev-dep | modify |
| `crates/okf-cli/src/main.rs` | `predicates`/`commit`/`log`/`restore`/`log-sync`/`set-active`/`get-active`/`register` + scaffold integration | modify |
| `crates/okf-mcp/src/main.rs` | MCP tools for the above | modify |
| `crates/okf-mcp/src/ops.rs` | scaffold integration (git+register+activate) | modify |
| `crates/okf-mcp/src/instructions.rs` | predicate line → 24 | modify |
...
**Files:** Modify `crates/okf-core/src/model.rs`; Modify `crates/okf-core/src/lib.rs:90`

**Interfaces — Produces:** `Predicate::UsedToStudy`; `PREDICATES: [&str; 24]` (adds `"used_to_study"` before `"reported_in"`).

- [ ] **Step 1 — failing test.** In `model.rs`, append a `#[cfg(test)] mod tests` (or add to lib.rs tests). Add to `crates/okf-core/src/lib.rs` test `node_type_palette_is_complete`, change the predicate assertion and add a round-trip:
...
- [ ] **Step 2 — run, expect fail.** `cargo test -p okf-core node_type_palette_is_complete` → FAIL (len is 23 / `used_to_study` parses to Unknown).
...
- [ ] **Step 4 — run, expect pass.** `cargo test -p okf-core node_type_palette_is_complete` → PASS.

- [ ] **Step 5 — commit.** `git add -A && git commit -m "feat(okf-core): add used_to_study predicate (24)"`

### Task 2: Domain/range for `used_to_study` + fix stale "23" messages

**Files:** Modify `crates/okf-core/src/lint.rs:160` and `:212-244`

**Interfaces — Consumes:** `Predicate::UsedToStudy` (Task 1).

- [ ] **Step 1 — failing test.** Add to `crates/okf-core/src/lint.rs` a `#[cfg(test)] mod tests` block (create if absent):
...
- [ ] **Step 2 — run, expect fail.** `cargo test -p okf-core used_to_study_range_violation_warns` → FAIL.
...
- [ ] **Step 4 — run, expect pass.** `cargo test -p okf-core used_to_study_range_violation_warns` → PASS. Also `cargo test -p okf-core` (whole crate) → PASS.

- [ ] **Step 5 — commit.** `git add -A && git commit -m "feat(okf-core): used_to_study domain/range lint + 24-predicate messaging"`

### Task 3: `okf predicates` CLI/MCP + sync instructions.md

**Files:** Modify `crates/okf-cli/src/main.rs`; Modify `crates/okf-mcp/src/main.rs`; Modify `crates/okf-mcp/src/instructions.rs`; Test `crates/okf-cli/tests/cli.rs`

**Interfaces — Produces:** CLI `okf predicates [--json]`; MCP `okf_predicates`.

- [ ] **Step 1 — failing test.** Append to `crates/okf-cli/tests/cli.rs`:
```rust
#[test]
fn predicates_lists_24() {
    let out = std::process::Command::new(env!("CARGO_BIN_EXE_okf"))
...
- [ ] **Step 2 — run, expect fail.** `cargo test -p okf-cli predicates_lists_24` → FAIL (no `predicates` subcommand).
...
- [ ] **Step 4 — run, expect pass.** `cargo test -p okf-cli predicates_lists_24` → PASS.

- [ ] **Step 5 — implement (MCP + instructions).** In `okf-mcp/src/main.rs`, add a tool method inside the `#[tool_router]` impl:
```rust
...
```
In `okf-mcp/src/instructions.rs`, update any "23 predicates"/"23 forward-only" wording to **24** and mention `used_to_study`.

- [ ] **Step 6 — verify + commit.** `cargo build && cargo test -p okf-cli` → PASS. `git add -A && git commit -m "feat(okf): okf predicates tool + 24-predicate instructions"`
...
**Files:** Modify `crates/okf-core/Cargo.toml`
...
tempfile = "3"
```
- [ ] **Step 2 — verify it resolves.** `cargo build -p okf-core` → succeeds (fetches `tempfile`; `uuid` already in lockfile). If offline and the fetch fails, run once with network.
- [ ] **Step 3 — commit.** `git add -A && git commit -m "build(okf-core): add uuid dep + tempfile dev-dep"`

### Task 5: `git.rs` — repo lifecycle + commit + a date helper

**Files:** Create `crates/okf-core/src/git.rs`; Modify `crates/okf-core/src/lib.rs`
...
- [ ] **Step 1 — failing test.** Create `crates/okf-core/tests/version.rs`:
```rust
...
```
- [ ] **Step 2 — run, expect fail.** `cargo test -p okf-core --test version` → FAIL (no module).
- [ ] **Step 3 — implement.** Create `crates/okf-core/src/git.rs`:
```rust
...
- [ ] **Step 4 — run, expect pass.** `cargo test -p okf-core --test version && cargo test -p okf-core git::` → PASS.
- [ ] **Step 5 — commit.** `git add -A && git commit -m "feat(okf-core): git.rs repo lifecycle, commit_all, ChangeKind, today_iso"`

### Task 6: `git.rs` — `log()` and forward-only `restore_to()`

**Files:** Modify `crates/okf-core/src/git.rs`

**Interfaces — Produces:** `GitRepo::{log,restore_to}`.

- [ ] **Step 1 — failing test.** Append to `crates/okf-core/tests/version.rs`:
```rust
#[test]
...
```
- [ ] **Step 2 — run, expect fail.** `cargo test -p okf-core --test version log_parses_kind_and_restore_is_forward_only` → FAIL.
...
```
- [ ] **Step 4 — run, expect pass.** `cargo test -p okf-core --test version` → PASS.
- [ ] **Step 5 — commit.** `git add -A && git commit -m "feat(okf-core): git log parsing + forward-only restore"`

### Task 7: `git.rs` — transactions (branch + squash)

**Files:** Modify `crates/okf-core/src/git.rs`; Modify `crates/okf-core/src/lib.rs` (export `Txn`)
...
- [ ] **Step 4 — run, expect pass.** `cargo test -p okf-core --test version` → PASS.
- [ ] **Step 5 — commit.** `git add -A && git commit -m "feat(okf-core): git transactions (branch + squash-merge)"`

### Task 8: `log_sync.rs` — the sole step-committer

**Files:** Create `crates/okf-core/src/log_sync.rs`; Modify `crates/okf-core/src/lib.rs`
...
```
- [ ] **Step 2 — run, expect fail.** → FAIL.
- [ ] **Step 3 — implement.** Create `crates/okf-core/src/log_sync.rs`:
```rust
...
- [ ] **Step 4 — run, expect pass.** `cargo test -p okf-core --test version` → PASS.
- [ ] **Step 5 — commit.** `git add -A && git commit -m "feat(okf-core): log_sync sole step-committer (atomic log.md + commit)"`

### Task 9: CLI wiring for commit/log/restore/log-sync

**Files:** Modify `crates/okf-cli/src/main.rs`; Test `crates/okf-cli/tests/cli.rs`

**Interfaces — Produces:** `okf log-sync`, `okf commit`, `okf log`, `okf restore`.
...
fn cli_log_sync_then_log() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("log.md"), "# Change log\n").unwrap();
    let ok = std::process::Command::new(env!("CARGO_BIN_EXE_okf"))
        .args(["log-sync", dir.path().to_str().unwrap(), "--kind", "ingest", "--summary", "seed"])
        .output().unwrap();
    assert!(ok.status.success(), "{}", String::from_utf8_lossy(&ok.stderr));
    let log = std::process::Command::new(env!("CARGO_BIN_EXE_okf"))
...
```
Add `tempfile = "3"` to `crates/okf-cli/Cargo.toml` `[dev-dependencies]`.
...
```
- [ ] **Step 4 — run, expect pass.** `cargo test -p okf-cli cli_log_sync_then_log` → PASS.
- [ ] **Step 5 — commit.** `git add -A && git commit -m "feat(okf-cli): log-sync/commit/log/restore subcommands"`
...
**Files:** Create `crates/okf-core/src/registry.rs`; Modify `crates/okf-core/src/lib.rs`; Test `crates/okf-core/tests/active.rs`

**Interfaces — Produces:** `registry::{Base, register, unregister, list, resolve, validate_kb_id}`.

- [ ] **Step 1 — failing test.** Create `crates/okf-core/tests/active.rs`:
```rust
...
```
- [ ] **Step 2 — run, expect fail.** → FAIL.
- [ ] **Step 3 — implement.** Create `crates/okf-core/src/registry.rs`:
```rust
...
- [ ] **Step 4 — run, expect pass.** `cargo test -p okf-core --test active` → PASS.
- [ ] **Step 5 — commit.** `git add -A && git commit -m "feat(okf-core): registry.rs known-bundle registry + validate_kb_id"`

### Task 11: `active.rs` — active-KB pointer

**Files:** Create `crates/okf-core/src/active.rs`; Modify `crates/okf-core/src/lib.rs`

**Interfaces — Produces:** `active::{get_active, set_active}`.

- [ ] **Step 1 — failing test.** Append to `crates/okf-core/tests/active.rs`:
```rust
#[test]
...
```
- [ ] **Step 2 — run, expect fail.** → FAIL.
- [ ] **Step 3 — implement.** Create `crates/okf-core/src/active.rs`:
```rust
...
- [ ] **Step 4 — run, expect pass.** `cargo test -p okf-core --test active` → PASS.
- [ ] **Step 5 — commit.** `git add -A && git commit -m "feat(okf-core): active.rs active-KB pointer"`

### Task 12: CLI wiring for set-active/get-active/register + scaffold integration

**Files:** Modify `crates/okf-cli/src/main.rs`; Test `crates/okf-cli/tests/cli.rs`

**Interfaces — Produces:** `okf set-active`, `okf get-active`, `okf register`; `okf scaffold` now git-inits + registers + set-actives.
...
fn scaffold_registers_inits_and_activates() {
    ...
    let s = std::process::Command::new(env!("CARGO_BIN_EXE_okf"))
    ...
    let ga = std::process::Command::new(env!("CARGO_BIN_EXE_okf"))
...
fn cmd_get_active(root: PathBuf, json: bool) -> Result<()> {
    match okf_core::active::get_active(&root) {
        ...
        None => { if json { println!("{}", serde_json::json!({"id": null})); } else { println!("(no active KB — run `okf set-active`)"); } }
    }
    Ok(())
}
fn cmd_register(root: PathBuf, kb_id: Option<String>, path: Option<PathBuf>, list: bool, unregister: Option<String>) -> Result<()> {
    if list { for b in okf_core::registry::list(&root) { println!("{}  {}", b.id, b.path); } return Ok(()); }
    if let Some(id) = unregister { okf_core::registry::unregister(&root, &id).map_err(anyhow::Error::msg)?; return Ok(()); }
    match (kb_id, path) {
...
```rust
    ...
    let repo = okf_core::git::GitRepo::open(&path);
    let kb_id = okf_core::registry::validate_kb_id(
...
```
- [ ] **Step 4 — run, expect pass.** `cargo test -p okf-cli scaffold_registers_inits_and_activates` → PASS.
- [ ] **Step 5 — commit.** `git add -A && git commit -m "feat(okf-cli): set-active/get-active/register + scaffold git-init+register+activate"`

### Task 13: MCP tools (log-sync/log/restore/set-active/get-active) + scaffold integration

**Files:** Modify `crates/okf-mcp/src/main.rs`; Modify `crates/okf-mcp/src/ops.rs`

**Interfaces — Produces:** `mcp__biookf__okf_log_sync|okf_log|okf_restore|okf_set_active|okf_get_active`; `ops::scaffold` git-inits+registers+activates.

- [ ] **Step 1 — implement (ops.rs scaffold).** At the end of `ops::scaffold`, before `Ok(...)`, mirror the CLI integration (git ensure_repo + initial commit + register + set-active) using `okf_core::git`/`registry`/`active`. Reuse the same logic as Task 12 Step 3 (adapted to the `ops::scaffold(bundle, name)` signature; `root = bundle.parent()`).
...
```
```rust
    #[tool(name="okf_log_sync", description="Append a dated log.md entry AND commit atomically (the sole step-committer).")]
    ...
    #[tool(name="okf_restore", description="Forward-only restore to a prior commit sha.")]
    ...
    pub async fn set_active(&self, p: Parameters<RootIdParam>) -> Result<CallToolResult, rmcp::model::ErrorData> {
        match okf_core::active::set_active(Path::new(&p.0.root), Some(&p.0.kb_id)) { Ok(()) => ok(format!("active KB = {}", p.0.kb_id)), Err(e) => ok(format!("ERROR: {e}")) }
    }
    #[tool(name="okf_get_active", description="Get the active KB id + path under <root>.")]
...
```
- [ ] **Step 3 — verify build.** `cargo build` → succeeds (whole workspace, incl. okf-mcp).
- [ ] **Step 4 — commit.** `git add -A && git commit -m "feat(okf-mcp): version-tracking + active-KB MCP tools; scaffold integration"`
...
**Files:** Test `crates/okf-cli/tests/cli.rs`
...
fn end_to_end_scaffold_write_logsync_log_restore() {
    ...
    let okf = env!("CARGO_BIN_EXE_okf");
    let run = |args: &[&str]| std::process::Command::new(okf).args(args).output().unwrap();
...
```
- [ ] **Step 2 — run.** `cargo test -p okf-cli end_to_end_scaffold_write_logsync_log_restore` → PASS.
- [ ] **Step 3 — full suite + lint.** `cargo test` (all crates) → PASS; `cargo build` → clean; optionally `cargo clippy`.
- [ ] **Step 4 — commit.** `git add -A && git commit -m "test(okf): end-to-end foundation (scaffold→write→log-sync→log→restore)"`

---

## Self-Review

**Spec coverage:** SP0 → Tasks 1–3 (predicate add, domain/range, `okf predicates`, instructions sync). SP1 → Tasks 4–9 (deps, git lifecycle/commit/date, log+restore, txn, log_sync, CLI). SP2 → Tasks 10–13 (registry, active, CLI, MCP+scaffold). Integration → Task 14. All foundation deliverables in the spec map to a task.
