...
foundational sub-projects and treats SP3 to SP6 as out of scope (each gets its own spec).
...
**Why foundation first:** SP3 to SP6 all depend on a correct predicate vocabulary (SP0), a way to
...
4. **`bokf-mcp/src/instructions.rs`**: sync the predicate line (currently lists 23) to 24.
5. **New tool `bokf predicates [--json]`**: print the active controlled vocabulary (28 types, the
...
### Acceptance
- `bokf_core::PREDICATES.len() == 24`; `used_to_study` round-trips parse↔as_str.
...
- `bokf predicates --json` lists exactly 24 predicates incl. `used_to_study`.
...
### Problem
No version tracking exists (verified: no `git.rs`, zero git references in `bokf-core/src`). The user
...
ingest squash→`Ingest`, merge squash→`Merge`, `bokf index`→`Index`, lint fixes→`Lint`,
restore→`Restore`, scaffold/manual edits→`Manual`, link-only edits→`Link`.

**`bokf-core/src/log_sync.rs`, the sole *step* committer.** `log_sync(bundle, kind, summary, delta,
counts, txn)` appends a `## YYYY-MM-DD` block to `log.md` (seeding `# Log` if absent, newest-first
per the existing bokf convention) **and** commits, atomically, in one call. It is the only path that
produces a *logged curation-step* commit; the lower-level `bokf commit` handles **non-logged
...
node-by-type / edge / source-node counts from `bokf stats` to populate the Step-4/Step-3 required
...
> each skill step **explicitly** calling `bokf log-sync` with the right `--kind` and a descriptive
> message. Final log↔commit **parity** is verified later at the Stop gate (SP6) via `bokf verify`,
> which is the real guarantee since a PostToolUse hook cannot enforce it.

### CLI + MCP surface (this slice)
- CLI: `bokf log-sync <bundle> --kind K --summary S [--delta D] [--counts] [--txn LABEL]`,
  `bokf commit` (lower-level stage+commit, used by txn squash/restore), `bokf log [--limit N]
  [--json]`, `bokf restore <bundle> <sha> [--summary S]`.
- MCP (server name `biookf`): `mcp__biookf__bokf_log_sync`, `bokf_log`, `bokf_restore`.

### Acceptance
- `bokf scaffold` produces a bundle with a `.git` and an initial `[manual] create knowledge base
  <id>` commit.
- `bokf log-sync … --kind ingest --summary "…"` appends a dated `log.md` block **and** creates one
  matching `[ingest]` commit; `bokf log --json` reconstructs it with the right kind/summary/delta.
- A pre-existing bundle with **no** `.git` gets one on its first `log-sync` (init-on-first-use).
- `bokf restore <old-sha>` adds a new `[restore]` commit; history is never rewound.
...
### Components
**`bokf-core/src/registry.rs`**: `<root>/registry.yaml` = `{ bases: [ { id, path } ] }`, the known-
...
**`bokf-core/src/active.rs`**: `<root>/.active-kb` = plaintext kb-id of the active graph.
...
**Scaffold integration (critical sequencing fix):** `bokf scaffold` now also **registers** and
**set-actives** the newly created bundle (in addition to `git init`). Without this, the first
`bokf convert` after scaffold (SP3) would be denied by the future require-active hook.
...
### CLI + MCP surface (this slice)
- CLI: `bokf set-active <root> <kb-id>`, `bokf get-active <root> [--json]`,
  `bokf register <root> <kb-id> <path> | --list | --unregister <kb-id>`.
- MCP: `mcp__biookf__bokf_set_active`, `bokf_get_active`, and `bokf_list_bases` made active-aware.

> The **SessionStart hook** that injects "Active BioOKF KB: …" into agent context consumes
> `bokf get-active`, but the hook itself is wired in SP6. This slice ships the tool it depends on.

### Acceptance
- `bokf register` then `bokf set-active` then `bokf get-active --json` returns the id + resolved path;
  unset → `None`. Invalid kb-ids are rejected.
- `bokf scaffold foo --name Foo` leaves `foo` registered **and** active.
...
Each new `bokf-core` module has one responsibility and a small interface:

| Module | Responsibility | Depends on |
|---|---|---|
| `model.rs` (edit) | controlled vocab incl. the 24th predicate | none |
...
CLI (`bokf-cli/src/main.rs`) and MCP (`bokf-mcp/src/{main,ops}.rs`) are thin front-ends over these.
...
- **SP0 unit:** `PREDICATES.len()==24`; parse/as_str round-trip for `used_to_study`; a fixture doc
  ...
  `bokf predicates --json` snapshot.
- **SP1 unit/integration:** init-on-first-use on a bundle with no `.git`; `log_sync` produces one
  commit + one dated `log.md` block; `bokf log` parses kind/summary/delta; forward-only `restore`
...
SP3 conversion pipeline; SP4 lint-extensions + `bokf verify`; SP5 skills; SP6 hooks + plugin
